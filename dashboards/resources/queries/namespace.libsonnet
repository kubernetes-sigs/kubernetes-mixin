...

{
  ...
  cpuThrottling(config):: |||
    sum(increase(container_cpu_cfs_throttled_periods_total{%(cadvisorSelector)s, %(clusterLabel)s="$cluster", namespace="$namespace", container!=""}[%(grafanaIntervalVar)s])) by (pod)
    /
    sum(increase(container_cpu_cfs_periods_total{%(cadvisorSelector)s, %(clusterLabel)s="$cluster", namespace="$namespace", container!=""}[%(grafanaIntervalVar)s])) by (pod)
  ||| % config,

  cpuThrottledSeconds(config)::
    'sum(increase(container_cpu_cfs_throttled_seconds_total{%(cadvisorSelector)s, %(clusterLabel)s="$cluster", namespace="$namespace", container!=""}[%(grafanaIntervalVar)s])) by (pod)' % config,
