...

{
  ...
  threads(config)::
    'sum(max by (%(clusterLabel)s, %(namespaceLabel)s, pod, container)(container_threads{%(cadvisorSelector)s, %(clusterLabel)s="$cluster", namespace="$namespace", pod="$pod", container!=""})) by (container)' % config,
